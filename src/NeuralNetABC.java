 ...
 * inputs. Runs an A-B-C network and trains it.
 * @author Nathan Wang
 * @version February 1, 2020
 */
public class NeuralNetABC
{
   /**
    ...
    */
   public NeuralNetABC(int input, int[] hiddenLayer, int output)
   {
      gradient = true;
      testingAll = false;
   ...
   public void deltaWeights(int trainingTest)
   {
      for (int j = 0; j < verticalLayers[1]; j++)                //For First Layer Weights.
      {
         ...
         for (int i = 0; i < numOutputNodes; i++)
         {
            ...
            omegaI = expected[trainingTest][i] - fI;         //ωi = Ti - Fi

            double thetaI = 0.0;
            for (int k = 0; k < verticalLayers[0]; k++)
            {
               thetaI += activation[1][k] * weight[1][k][i];     //unthresholded hidden layer value.
            }
         ...
         for (int k = 0; k < verticalLayers[0]; k++)
         {
            thetaJ += activation[0][k] * weight[0][k][j];
         }
      ...
      {
         double thetaHidden = 0.0;                                    // Below is for Second layer Weights.
         ...
         {
            thetaHidden += activation[1][j] * weight[1][j][i];        //Same theta0
         }
         double psiI = omegaI * derivativeThreshold(thetaHidden);     //ψ0 = ω0 * derivative of theta0

         for (int j = 0; j < verticalLayers[1]; j++)             //iterates through each hidden layer node
         {
            double hJ = activation[1][j];                        //Thresholded thetaJ
            double derivErrorJI = hJ * psiI;                     //derivJ0 = -hJ * ψ0
   ...
   public static void main(String[] args)
   {
      ...
      NeuralNetABC neuro = new NeuralNetABC(inputLayers, hiddenLayer, outputLayers); //Sets inputs inside constructor.
